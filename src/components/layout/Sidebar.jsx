...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <aside
      className={`bg-gray-800 text-white transition-all duration-300 ease-in-out transform ${isOpen ? 'w-64' : 'w-20'
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-gray-700">
        <div className="flex items-center">
          <div className="flex-shrink-0 flex items-center">
            ...
            <span
              className={`ml-2 font-bold text-lg whitespace-nowrap text-[#FEAB5F] ${!isOpen && 'hidden'
                }`}
            >
              NativoES
            </span>
        ...
        <div className="space-y-1">
          {itemsToRender.map((item, index) => (
            <Link
              key={index}
              href={item.pathname}
              className={`group flex items-center px-2 py-3 text-sm font-medium rounded-md transition-all duration-150 ease-in-out ${item.active
            ...
            >
              ...
              <span
                className={`truncate transition-opacity duration-150 ${!isOpen && 'opacity-0 hidden'
                  }`}
              >
                {item.name}
              </span>
      ...
      <div
        className={`absolute bottom-0 w-full p-4 border-t border-gray-700 ${!isOpen && 'text-center'
          }`}
      >
        <div className="flex items-center">
