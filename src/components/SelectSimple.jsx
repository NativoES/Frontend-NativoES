...
export default function Select({ arr, name, click, defaultValue, uuid, label, position, bg, required, diferent }) {
    ...
    const { select, setSelect, setNavItem, } = useAppContext()
    ...
    return (
        <div className='relative'>
            <div id={label}
                className={`relative  border border-[#a1a1a1] cursor-pointer pt-2.5 mb-0   w-full text-[12px]   px-5 ${bg ? `${bg} pb-2` : 'bg-transparent text-gray-900 border-[#a1a1a1]'}   focus:outline-none focus:ring-0  peer rounded-[5px]`}
                onClick={handlerSelect}
                >
                {defaultValue === 'Seleccionar' && <span className='absolute'>Seleccionar</span>}
                <input type="text" disabled className='relative inline  h-full border-transparent outline-none focus:outline-none bg-transparent' value={defaultValue !== undefined && defaultValue !== 'Seleccionar' ? defaultValue : ''} minLength={2} required={required} />
